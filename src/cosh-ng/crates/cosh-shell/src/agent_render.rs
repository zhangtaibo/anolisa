...
pub use crate::approval_actions::{
...
impl RatatuiInlineRenderer {
    ...
    pub fn governed_event_lines(&self, governed_events: &[GovernedEvent]) -> Vec<String> {
        self.render_lines(
            lines_from_governed_events(governed_events),
            self.content_width(),
        )
    }
    ...
    pub fn write_loading<W: Write>(&self, output: &mut W) -> io::Result<()> {
        self.write_block(output, "Agent", vec!["Thinking...".to_string()], None)
    }
    ...
    pub fn stream_agent(&self) -> StreamingAgentBlock {
        StreamingAgentBlock::new(self.content_width(), self.plain)
    }
    ...
    pub fn write_agent_response<W: Write>(
        ...
        let model = MarkdownRenderModel::parse(text, self.content_width());
        if self.styled && !self.plain {
            let mut body = model.styled_lines();
            if let Some(footer) = footer {
                body.extend(
                    MarkdownRenderModel::parse(footer, self.content_width()).styled_lines(),
                );
            }
            return self.write_styled_block(output, "Agent", body);
        }
        ...
        self.write_block(output, "Agent", body, footer)
    }
...
fn lines_from_governed_events(governed_events: &[GovernedEvent]) -> Vec<Line<'static>> {
    let mut lines = Vec::new();
    for event in governed_events {
        for (idx, line) in display_lines_for_event(event).into_iter().enumerate() {
...
fn display_lines_for_event(event: &GovernedEvent) -> Vec<String> {
    match &event.event {
